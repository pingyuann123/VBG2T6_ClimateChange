document.addEventListener('DOMContentLoaded', function () {
    ...
    function makeChart(data) {
        ...
        const scatter = plotArea.selectAll("circle")
            ...
            .on("mouseover", function (event, d) {
                tooltip.html(`${d.Entity}<br>Emissions: ${d3.format(".2f")(d.Emissions)} t CO₂e<br>Urban Rate: ${d3.format(".2f")(d.UrbanRate)}%`)
            ...
            .on("click", function (event, d) {
                d3.selectAll("circle")
                    .attr("stroke", "#ccc") // Reset all circle strokes
                    .attr("stroke-width", 1);
    ...
    d3.csv("Datasets/combines.csv").then(function (rawData) {
        ...
        populateCountryFilter(averagedData);

        // Initial chart rendering
        makeChart(averagedData);

        // Filter data when country selection changes
    ...
    function populateCountryFilter(data) {
        ...
        const uniqueEntities = [...new Set(data.map(d => d.Entity))]; // Get unique countries
        ...
        countrySelect.prepend(optionAll);
    }

    // Update the chart based on selected country filter
    function updateChart(data) {
        const selectedCountry = document.getElementById('countryFilter').value;

        // Filter data based on selected country
        const filteredData = selectedCountry === 'All' ? data : data.filter(d => d.Entity === selectedCountry);
